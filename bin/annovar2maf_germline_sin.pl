...
my $type = shift or die $!;  #Mutect2 UKB Pindel LoFreq
...
while(<IN>){
	...
	for (my $i=126; $i<@tmp; $i++){
		...
		my $samid;
		if($type eq "UKB"){
			$samid = $1 if( $tmp[124] =~ /SAM=(\S+)/ );
		}else{
			$samid = $sam_id[$j];
		}
		my @alt = split(/,/, $tmp[100]);
        # my @gt =  split(/[:\/\|]/ , $tmp[$i]);
		# if(@alt >= 2){
		#     my $alt_s  = @alt[$gt[1]-1];
		#     my($ref2,$alt2);
		#     if(length($ref) > length($alt_s)){
		#         $ref2 = substr($ref, length($alt_s));
		#         $alt2 = "-";
		#     }elsif(length($ref) < length($alt_s)){
		#         $ref2 = "-";
		#         $alt2 =  substr($alt_s, length($ref));
		#     }else{
		#         $ref2 = $ref;
		#         $alt2 = $alt_s;
		#     }
		#     next if (($tmp[3] ne "$ref2") || ($tmp[4] ne "$alt2"));
		# }
		my($ref_r,$alt_r);
		my @AD;
		if($type eq "Mutect2"){
