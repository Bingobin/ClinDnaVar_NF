...
process VarDict_Call {
    ...
    awk 'BEGIN{OFS="\\t"}  /^#/ {print; next} \$4 ~ /^[ACGTN]+\$/ && \$5 ~ /^([ACGTN]+)(,[ACGTN]+)*\$/ {print}' ${sample_id}.vardict.vcf > ${sample_id}.vardict.clean.vcf && mv ${sample_id}.vardict.clean.vcf ${sample_id}.vardict.vcf
